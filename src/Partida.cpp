...
void Partida::moverJugador(int tipoMovimiento) {
    ...
    if (tipoCasilla == "Enemigo"){                                                                                          //1
        ...
        trampasPartida->insertar(casillaDestino.getUbicacion());                                                       //1
            ...
            jugador.setVida(jugador.getVida() + casillaDestino.getEfecto());                                                //1
        cout << "Encontraste una pocima :D!! recuperas " << casillaDestino.getEfecto() << " puntos de vida." << endl;       //1                                                                                            //1
    } else if (tipoCasilla == "Pista"){                                                                                     //1
        int distancia= calcularDistanciaPista(nuevoX, nuevoY, nuevoZ);                                                      //1
    ...
    reemplazarCasillaVacia(nuevoX, nuevoY, nuevoZ);                                                                         //1
...
int Partida::calcularDistanciaPista(int nuevoX, int nuevoY, int nuevoZ) {
    ...
    //convirtiendo distancias a valores positivos (si los son)                                                      //1
    if (distanciaX < 0) {                                                                                           //1
        distanciaX *= -1;                                                                                           //1
    }
    if (distanciaY < 0) {                                                                                           //1
